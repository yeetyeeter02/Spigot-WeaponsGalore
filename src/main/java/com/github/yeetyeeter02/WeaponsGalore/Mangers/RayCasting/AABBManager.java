...
public class AABBManager {
    ...
    public AABBManager(Vector min, Vector max) {
        this(min.getX(), min.getY(), min.getZ(), max.getX(), max.getY(), max.getZ());
    }

    // Main constructor for AABB
    public AABBManager(double x1, double y1, double z1, double x2, double y2, double z2) {
    ...
    public boolean collides(RayTraceManager ray, double tmin, double tmax) {
        for (int i = 0; i < 3; i++) {
    ...
    public double collidesD(RayTraceManager ray, double tmin, double tmax) {
        for (int i = 0; i < 3; i++) {
            double d = 1 / ray.direction(i);
            double t0 = (min(i) - ray.origin(i)) * d;
            double t1 = (max(i) - ray.origin(i)) * d;
    ...
    public boolean contains(Location location) {
        if (location.getX() > max.getX()
            ...
            && location.getY() < min.getY()) { return false; }
        return true;
    }
}
